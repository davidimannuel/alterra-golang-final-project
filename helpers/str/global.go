...
// NilString ...
// return nil value instead empty string
func NilString(text *string) *string {
	if text != nil {
		if *text == "" {
			return nil
		}
...
// DefaultUUID ...
func DefaultUUID(text string) string {
	if !IsValidUUID(text) {
...
// DefaultDate ...
func DefaultDate(text string) string {
	if text == "" {
...
// AddQuotesToEachWord ...
func AddQuotesToEachWord(word string, delimeter string) string {
	var concate string
	...
	for _, word := range words {
		concate += `'` + word + `'` + delimeter
	}

	return strings.TrimSuffix(concate, delimeter)
}
